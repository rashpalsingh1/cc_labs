...
```{r}
#check the number of times the thougt police are mentioned
whole_text %>% 
  str_extract_all("[Tt][Hh][Oo][Uu][Gg][Hh][Tt][:blank:][Pp][Oo][Ll][Ii][Cc][Ee]") %>% 
  flatten_chr() %>% 
  length()
...
```{r}
whole_text %>%
  str_extract_all("[Bb][Ii][Gg][:blank:][Bb][Rr][Oo][Tt][Hh][Ee][Rr]") %>% 
  flatten_chr() %>% 
  length()
```
...
```{r}
#check any combination of case used for Julia
whole_text %>%
  str_detect("[Jj][Uu][Ll][Ii][Aa]")
```
...
```{r}
whole_text %>%
  str_extract_all("[A-Za-z]+[:blank:][:digit:]+[A-Za-z]+[:punct:][:blank:][:digit:]{4}")
```

Q5. Winston wants some anonymity, redact his name somehow.
```{r}
#check how many times Winston is mentioned
whole_text %>%
  str_extract_all("[Ww][Ii][Nn][Ss][Tt][Oo][Nn]") %>% 
  flatten_chr() %>% 
  length()

#32 times

# redact his name and store in a new variable
text_redacted <- whole_text %>%
  str_replace_all("[Ww][Ii][Nn][Ss][Tt][Oo][Nn]", "REDACTED")

#check how many times Winston appears
text_redacted %>%
  str_extract_all("[Ww][Ii][Nn][Ss][Tt][Oo][Nn]") %>% 
  flatten_chr() %>% 
  length()
...
Q6. Stocks of Victory Gin have run out and the people of Oceania will have to drink vodka instead. Replace “gin” with “vodka”, matching case wherever possible. There’s still loads of Victory Coffee, though, so don’t replace that.

```{r}
#check on the case usage for the word gin
whole_text %>%
  str_extract_all("[ .,][Gg][Ii][Nn][ .,]")
# 3 in total, one upper case

#only two different cases used in the text for the word 'gin', replace both with 'vodka' preserving the case used
text_no_gin  <- whole_text %>%
  str_replace(" GIN.", "VODKA") %>% 
  str_replace_all(" gin.", "vodka") %>% 
  str_replace_all(" gin ", "vodka")

#check
text_no_gin %>% 
  str_extract_all("[Vv][Oo][Dd][Kk][Aa]")
```



Q7. Big Brother is rebranding and has decided that the various ministries will now be known as departments. The old departments are now called sections. The Newspeak titles will use “Dep” instead of “Mini”.

```{r}
whole_text %>%
  str_extract_all("[A-Za-z]+[:blank:][Mm][Ii][Nn][Ii][Ss][Tt][Rr][Ii][Ee][Ss]")

whole_text %>%
  str_extract_all("[Dd][Ee][Pp][Aa][Rr][Tt][Mm][Ee][Nn][Tt]")

whole_text %>%
  str_extract_all("[Mm][Ii][Nn][Ii]")


rebranded <- whole_text %>%
  str_replace_all("[Dd][Ee][Pp][Aa][Rr][Tt][Mm][Ee][Nn][Tt]", "sections") %>% 
  str_replace_all("[Mm][Ii][Nn][Ii][Ss][Tt][Rr][Ii][Ee][Ss]", "departments") %>% 
  str_replace_all("[Mm][Ii][Nn][Ii]", "Dep")
